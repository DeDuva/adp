...
const TEST_KEY = "webhooks-test-credential-key";
  ...
  it("delivers only to active hooks subscribed to the fired event type", async () => {
    ...
    const db = {
      select: () => ({
        from: () => ({
          where: async () => [
            { targetUrl: "http://sub-a.example/hook", secretCiphertext: encryptCredential("s", TEST_KEY), events: ["push"], active: true },
            { targetUrl: "http://sub-b.example/hook", secretCiphertext: encryptCredential("s", TEST_KEY), events: ["pull_request"], active: true },
            { targetUrl: "http://sub-c.example/hook", secretCiphertext: encryptCredential("s", TEST_KEY), events: ["push"], active: true },
          ],
        }),
      }),
    ...
    emitWebhookEvent(db, "repo-1", "push", { hello: "world" }, nullLogger(), TEST_KEY);
  ...
  it("retries a failing delivery and eventually logs instead of throwing", async () => {
    ...
    const db = {
      select: () => ({
        from: () => ({
          where: async () => [
            { targetUrl: "http://flaky.example/hook", secretCiphertext: encryptCredential("s", TEST_KEY), events: ["push"], active: true },
          ],
        }),
      }),
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    emitWebhookEvent(db, "repo-1", "push", {}, logger, TEST_KEY);
