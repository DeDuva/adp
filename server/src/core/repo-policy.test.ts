...
import { resolveLandRequirements, EMPTY_POLICY, type RepoPolicy } from "./repo-policy.js";
...
describe("resolveLandRequirements", () => {
  ...
  it("unions the floor with the repo's own requirements", () => {
    const repoPolicy: RepoPolicy = { gates: ["tests"], land: { require: ["one_approval"] } };
  ...
  it("a repo can only add requirements, never remove a floor one", () => {
    const repoPolicy: RepoPolicy = { gates: [], land: { require: [] } };
  ...
  it("dedups when both the floor and the repo name the same requirement", () => {
    const repoPolicy: RepoPolicy = { gates: [], land: { require: ["gates_green"] } };
