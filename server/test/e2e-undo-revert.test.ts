...
describe.skipIf(skipWithoutDb)("#159: compensating-revert undo", () => {
  ...
  beforeAll(async () => {
    ...
    const [reviewer] = await db
      ...
      .returning();
    reviewerToken = await mintToken(db, reviewer!.id, ["repo:read", "repo:write"]);
  ...
  async function repoWithMovedBranch(
    ...
    const dir = await mkdtemp(path.join(tmpdir(), `adp-e2e-revert-${repoName}-`));
    ...
    const write = async (files: { file: string; content: string }[]) => {
      for (const f of files) {
        await execFileAsync("sh", ["-c", `printf '%b' ${JSON.stringify(f.content)} > ${f.file}`], { cwd: dir });
      }
      await execFileAsync("git", ["add", "-A"], { cwd: dir });
    };

    if (adpYaml) await execFileAsync("sh", ["-c", `printf '%b' ${JSON.stringify(adpYaml)} > adp.yaml`], { cwd: dir });
    await write(seed);
    ...
    await write(feature);
    await execFileAsync("git", ["commit", "-m", "the change we will regret"], { cwd: dir });
    await execFileAsync("git", ["push", "origin", "feature"], { cwd: dir });
    const featureSha = (await execFileAsync("git", ["rev-parse", "HEAD"], { cwd: dir })).stdout.trim();

    const created = await api(`/api/v3/repos/${owner}/${repoName}/pulls`, {
      method: "POST",
      body: JSON.stringify({ title: "Ship the thing", head: "feature", base: "main" }),
    });
    ...
    await api(`/api/v3/repos/${owner}/${repoName}/gates`, {
      method: "POST",
      body: JSON.stringify({ git_sha: featureSha, name: "tests", status: "success", summary: "green" }),
    });
    const merged = await api(`/api/v3/repos/${owner}/${repoName}/pulls/${number}/merge`, {
      method: "PUT",
      body: "{}",
    });
    ...
    await execFileAsync("git", ["push", "origin", "main"], { cwd: dir });
    const laterSha = (await execFileAsync("git", ["rev-parse", "main"], { cwd: dir })).stdout.trim();
    await rm(dir, { recursive: true, force: true });

    const ops = await api(`/api/adp/repos/${owner}/${repoName}/operations?verb=proposal.merge`);
  ...
  it("opens a revert proposal when the branch has moved, and leaves the branch alone", async () => {
    ...
    const undone = await api(`/api/adp/repos/${owner}/${repoName}/operations/${mergeOp.id}/undo`, {
      method: "POST",
      body: "{}",
    });
    ...
    const body = undone.body as unknown as {
      ...
      parent_op: string;
      undo_path: string;
      proposal: { number: number; head_ref: string; head_sha: string; base_ref: string; state: string };
    };
    ...
    expect(await gitBackend.resolveRef(owner, repoName, body.proposal.head_ref)).toBe(body.proposal.head_sha);
    ...
    expect(await gitBackend.statPath(owner, repoName, body.proposal.head_sha, "feature.txt")).toBeNull();
    expect(await gitBackend.statPath(owner, repoName, body.proposal.head_sha, "other.txt")).not.toBeNull();
  });
  ...
  it("subjects the revert to the same land policy, and lands it once the gate is green", async () => {
    ...
    const proposal = (undone.body as unknown as { proposal: { number: number; head_sha: string } }).proposal;
    ...
    const blocked = await api(`/api/v3/repos/${owner}/${repoName}/pulls/${proposal.number}/merge`, {
      method: "PUT",
      body: "{}",
    });
    expect(blocked.status).toBe(422);
    expect((blocked.body as unknown as { unmet: string[] }).unmet.join(" ")).toMatch(/gates_green/);

    await api(`/api/v3/repos/${owner}/${repoName}/gates`, {
      method: "POST",
      body: JSON.stringify({ git_sha: proposal.head_sha, name: "tests", status: "success", summary: "green" }),
    });
    const landed = await api(`/api/v3/repos/${owner}/${repoName}/pulls/${proposal.number}/merge`, {
      method: "PUT",
      body: "{}",
    });
    ...
    expect(await gitBackend.statPath(owner, repoName, head, "feature.txt")).toBeNull();
    expect(await gitBackend.statPath(owner, repoName, head, "other.txt")).not.toBeNull();
  });
  ...
  it("enqueues the revert's own gates, so the policy it must satisfy is satisfiable", async () => {
    ...
    const body = undone.body as unknown as { gate_jobs_enqueued: number; proposal: { head_sha: string } };
    ...
    const queued = await db
      .select()
      .from(gateJobs)
      .where(and(eq(gateJobs.gitSha, body.proposal.head_sha), eq(gateJobs.name, "tests")));
  ...
  it("refuses a conflicting revert with the paths named, rather than producing a broken tree", async () => {
    ...
    const body = undone.body as unknown as { message: string; conflicts: string[] };
    ...
    const proposals = await api(`/api/v3/repos/${owner}/${repoName}/pulls?state=all`);
    expect((proposals.body as unknown as unknown[]).length).toBe(1);
    expect(await gitBackend.resolveRef(owner, repoName, "adp/revert-1")).toBeNull();
  });
  ...
  it("refuses a second undo of the same merge, whichever path the first took", async () => {
    ...
    const first = await api(`/api/adp/repos/${owner}/${repoName}/operations/${mergeOp.id}/undo`, {
      method: "POST",
      body: "{}",
    });
    expect(first.status).toBe(200);

    const second = await api(`/api/adp/repos/${owner}/${repoName}/operations/${mergeOp.id}/undo`, {
      method: "POST",
      body: "{}",
    });
    expect(second.status).toBe(422);
    expect((second.body as unknown as { message: string }).message).toMatch(/already been undone/);
  });
  ...
  it("refuses when the merge is no longer in the branch's history at all", async () => {
    ...
    const seedSha = (await gitBackend.log(owner, repoName, "main", 100)).at(-1)!.sha;
    ...
    expect(undone.status).toBe(422);
    expect((undone.body as unknown as { message: string }).message).toMatch(/history was rewritten/);
  });
  ...
  it("still prefers the exact path: an untouched branch is rolled back, not reverted", async () => {
    ...
    await execFileAsync("git", ["commit", "-m", "seed"], { cwd: dir });
    await execFileAsync("git", ["push", "origin", "main"], { cwd: dir });
    const baseBefore = (await execFileAsync("git", ["rev-parse", "HEAD"], { cwd: dir })).stdout.trim();
    ...
    await api(`/api/v3/repos/${owner}/${repoName}/pulls/${number}/merge`, { method: "PUT", body: "{}" });
    ...
    expect(undone.status).toBe(200);
    const body = undone.body as unknown as { undo_path: string; verb: string; proposal?: unknown };
    ...
    expect(await gitBackend.resolveRef(owner, repoName, "main")).toBe(baseBefore);
  });
});
