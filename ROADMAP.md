...
| M4 — multi-tenant hosted preview | in progress | Work plan (M4-0 … M4-12) in [`docs/m4-readiness-review.md`](docs/m4-readiness-review.md). Track A underway: M4-0…M4-4 (org schema, org-scoped tokens, org policy plane, quotas/GC, audit-log export) landed. M4-9 (runner) complete — all four slices landed: M4-9a (gate-job queue), M4-9b (`runner/` package — isolated Docker executor), M4-9c (adp.yaml parsing + gate_results wiring), M4-9d (per-org concurrency caps). M4-11 (observability) and M4-12 (self-host artifacts) remain in Track A, no decision needed. Blocked on three author decisions for the rest of Track B — see Blockers |
...
- **Now:** M4 Track A in progress (`docs/m4-readiness-review.md`) — M4-0…M4-4 landed
  ...
  landed queue-first across four slices: M4-9a (gate-job queue, `FOR UPDATE SKIP LOCKED`),
  M4-9b (the `runner/` package — a pure-HTTP-client Docker executor with network-deny, no
  host mounts, no ambient secrets, CPU/memory/wall-clock caps), M4-9c (adp.yaml's
  `runner.gates` auto-enqueues a job on push; a checkout tarball — `git archive`,
  `docker cp`, never a bind mount — lets the executor build/test the pushed sha; completion
  produces real, signed `gate_results` evidence), M4-9d (per-org concurrency caps, scoped at
  claim time so one org at its cap never starves a different org's queue). M4-11
  (observability) and M4-12 (self-host artifacts) remain in Track A, no decision needed.
