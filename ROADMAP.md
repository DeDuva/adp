...
- **Now:** M4 Track A in progress (`docs/m4-readiness-review.md`) — M4-0…M4-4 landed
  ...
  isolated from the start, sliced queue-first: M4-9a (gate-job queue) and M4-9b (the
  `runner/` package — a pure-HTTP-client Docker executor with network-deny, no host mounts,
  no ambient secrets, CPU/memory/wall-clock caps) landed. M4-9c (adp.yaml's `runner.gates`
  ...
  `gate_results` evidence) in flight. M4-9d (per-org concurrency caps) remains. M4-11
  (observability) and M4-12 (self-host artifacts) remain, no decision needed.
